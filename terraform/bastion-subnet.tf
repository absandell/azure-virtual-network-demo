...
resource "azurerm_subnet_network_security_group_association" "bastion_subnet_nsg_association" {
    depends_on = [ azurerm_network_security_rule.bastion_nsg_rule_inbound ] # NSG rules need to be created in order to link
    subnet_id                 = azurerm_subnet.bastion_subnet.id
    network_security_group_id = azurerm_network_security_group.bastion_subnet_nsg.id
}

# NSG Inbound Rules [22, 3389]
resource "azurerm_network_security_rule" "bastion_nsg_rule_inbound" {
    for_each = bastion_inbound_ports_map
